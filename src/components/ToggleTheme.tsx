...
import { ToggleThemeContainer } from './ToggleThemeStyles'

import { ThemeContext } from '@/context'

/**
 * Componente funcional React que permite alternar el tema de la aplicación entre 'light' y 'dark'.
 * Utiliza el contexto de tema (`ThemeContext`) para obtener y modificar el estado actual del tema.
 *
 * El componente muestra un interruptor visual (toggle) con iconos
 * de sol y luna para indicar el estado del tema.
 * Al hacer clic en el interruptor, se cambia el tema de la aplicación.
 *
 * @returns {JSX.Element} Elemento JSX que representa el interruptor de cambio de tema.
 */
const ToggleTheme: React.FC = () => {
  const themeContext = useContext(ThemeContext)

  const { theme, setThemeUse } = themeContext!

  /**
   * Cambia el tema actual de la aplicación entre 'light' y 'dark'.
   * Si el tema actual es 'light', lo cambia a 'dark', y viceversa.
   *
   * @returns {void} No retorna ningún valor.
   */
  const changeTheme = (): void => {
    setThemeUse(theme === 'light' ? 'dark' : 'light')
  }
